...
sealed class ObstacleVo<T> {
  ...
  void selectObstacle(String value, BuildContext context);
...
class ObstacleSkate extends ObstacleVo<ModalitySpot> {
  ...
  factory ObstacleSkate.fromList(List<String> value) {
    ...
    if (withoutDuplicates.length < value.length) {
      throw ArgumentError(
          "Lista de obstáculos inválida, alguns elementos estão repetidos",
          value.where((test) => test == test).toString());
    }

    if (!_obstaclesAll.toSet().containsAll(value)) {
      throw ArgumentError('Lista de obstáculos com elementos fora do padrão',
          value.where((test) => _obstaclesAll.contains(test)).toString());
    }
  ...
  void selectObstacle(String value, BuildContext context) {
    ...
    if (_selectedObstacles.contains(value)) {
      SnackbarUtils.showSnackbarError(
          context, "Esse obstáculo já foi adicionado");
      return;
    }

    _selectedObstacles.add(value);
    debugPrint("Selecionou o obstáculo: $value");
  }
