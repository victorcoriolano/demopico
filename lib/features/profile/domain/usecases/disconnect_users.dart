...
class DisconnectUsers {
  static DisconnectUsers? _instance;
  static DisconnectUsers get instance {
    _instance ??= DisconnectUsers(
  ...
  Future<void> execute(Relationship connection) async {
    ...
    profileAddressedUser.profile!.connections.remove(connection.requesterUser.id);
    profileRequesterUser.profile!.connections.remove(connection.addressed.id);
